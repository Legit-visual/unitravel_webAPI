...
namespace unitravel_webAPI.Services.Implementations
{
    public class CancelService : ICancelService
    {
        ...
        public CancelService(HttpClient httpClient, IOptions<ApiCredentials> credentials)
        {
        ...
        public async Task<CancelResponse?> CancelAsync(CancelRequest request)
        {
            ...
            var response = await _httpClient.PostAsync($"{_credentials.BaseUrl}/Cancel", content);

            if (!response.IsSuccessStatusCode)
                throw new HttpRequestException($"Error {response.StatusCode}\n{response.Content}");

            var raw = await response.Content.ReadAsStringAsync();
            var result = JsonConvert.DeserializeObject<CancelResponse>(raw);
